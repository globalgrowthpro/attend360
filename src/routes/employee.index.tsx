  ...
  return (
    <EmployeeShell title="Employee home">
      <div className="space-y-4">
        <div>
          <p className="text-xl font-semibold">{t("Good Morning, Ahmed 👋")}</p>
          <p className="text-sm text-muted-foreground">{t("Thursday, 03 September 2026")}</p>
        </div>

        <Card className="overflow-hidden">
          <div className="bg-primary px-5 py-6 text-center text-primary-foreground">
            <p className="text-xs uppercase tracking-wide opacity-80">{t("Current time")}</p>
            <p className="mt-1 text-4xl font-semibold tabular">{clock}</p>
            <p className="mt-1 text-xs opacity-80">{t("Cairo HQ · Device PC-001 approved")}</p>
          </div>
          <CardContent className="space-y-4 p-5">
            <div className="grid grid-cols-3 gap-3 text-center">
              ...
              <div className="rounded-xl border border-border p-3">
                <p className="text-xs text-muted-foreground">{t("Check In")}</p>
              ...
              <div className="rounded-xl border border-border p-3">
                <p className="text-xs text-muted-foreground">
                  {phase === "done" ? t("Check Out") : t("Working")}
                </p>
                <p className="mt-0.5 text-sm font-medium tabular">
            ...
            {phase === "done" ? (
              <div className="rounded-xl bg-success-soft p-4 text-center">
                <p className="text-sm font-semibold text-success">{t("✓ Attendance Completed")}</p>
                <div className="mt-3 space-y-1 text-sm">
                  <div className="flex justify-between"><span className="text-muted-foreground">{t("Check In")}</span><span className="tabular">08:56 AM</span></div>
                  <div className="flex justify-between"><span className="text-muted-foreground">{t("Check Out")}</span><span className="tabular">05:05 PM</span></div>
                  <div className="flex justify-between font-medium"><span>{t("Total")}</span><span className="tabular">08h 09m</span></div>
                </div>
              </div>
            ) : (
              <Button
              ...
              >
                {phase === "idle" ? t("CHECK IN") : t("CHECK OUT")}
              </Button>
            )}

            <div className="flex items-center justify-center gap-2">
              <StatusBadge status={phase === "idle" ? "absent" : "present"} />
              <span className="text-xs text-muted-foreground">{t("Grace period 10 min")}</span>
            </div>
          </CardContent>
        </Card>

        <Card>
          <CardHeader><CardTitle className="text-base">{t("Validation checks")}</CardTitle></CardHeader>
              ...
              return (
                <div key={label as string} className="flex items-center justify-between">
                  <span className="flex items-center gap-2 text-muted-foreground">
                    <I className="size-4" /> {t(label as string)}
                  </span>
                  <span className="font-medium text-success">{t(value as string)}</span>
                </div>
              );
            })}
          </CardContent>
        ...
        <Card>
          <CardContent className="p-4">
            <div className="flex items-center justify-between gap-3">
              <div>
                <p className="text-sm font-medium">{t("Something wrong with a record?")}</p>
                <p className="text-xs text-muted-foreground">{t("Send a correction request to HR.")}</p>
              </div>
              <Button asChild variant="outline" size="sm">
                <Link to="/employee/correction">
                  <ClipboardPlus className="size-4" /> {t("Request")}
                </Link>
              </Button>
            </div>
